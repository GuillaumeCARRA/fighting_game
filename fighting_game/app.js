...
class Player {
  ...
  strike (player, enemy, attackDmg) {
    ...
    let dmgAmount = Math.ceil(Math.random() * attackDmg);
    // Subtract the enemy health with the damageAmount
    enemy.health -= dmgAmount;
    //  Update the game and DOM with updateGame()
    updateGame(p1, p2, game.isOver);
    //  Return a message of 'player name attacks enemy name for damageAmount'
    return `${player.name} attacks ${enemy.name} for ${dmgAmount}`;
  }
...
// name, health and attackdmg 
let player1 = new Player("Guillaume", 100, 10);

let player2 = new Player('IA', 100, 10); 
...
// we give to the function the player because i use argument
updateGame(p1, p2, game.isOver);
...
// Add functionality where players can press a button to attack OR heala
// ** Player 1 Controls **
